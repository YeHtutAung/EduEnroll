...
import { ORGANISER_TERMS_MAX } from "@/lib/legal/terms";
...
export default function IntakeDetailPage({
  ...
  const [editOrganiserTerms, setEditOrganiserTerms] = useState("");
  ...
  function openEditIntake() {
    ...
    setEditOrganiserTerms(intake.organiser_terms ?? "");
  ...
  async function handleSaveIntake(e: React.FormEvent) {
    ...
    try {
      // organiser_terms is always sent: blank clears it (the server trims and
      // stores empty as null), so an organiser can remove their rules.
      const payload: Record<string, unknown> = {
        name: editName.trim(),
        year: editYear,
        organiser_terms: editOrganiserTerms,
      };
          ...
          <div className="relative w-full max-w-md bg-white rounded-2xl shadow-xl p-6 max-h-[90vh] overflow-y-auto">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">Update name, year or event rules</p>
              </div>
              <button
                onClick={() => setEditingIntake(false)}
                className="ml-auto p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-gray-100 transition-colors"
            ...
            <form onSubmit={handleSaveIntake} className="space-y-4">
              ...
              {/* Organiser rules — shown to buyers, who must accept them to order */}
              <div className="border-t border-gray-100 pt-4">
                <label className="block text-sm font-medium text-gray-700 mb-1.5">
                  Event Rules Shown to Buyers
                </label>
                <textarea
                  value={editOrganiserTerms}
                  onChange={(e) => setEditOrganiserTerms(e.target.value)}
                  maxLength={ORGANISER_TERMS_MAX}
                  rows={5}
                  placeholder="e.g. Ages 18+. Bags are searched at entry. No re-entry."
                  className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a3f8a] focus:border-transparent"
                />
                <p className="mt-1.5 text-xs text-gray-400">
                  Buyers see these above the agreement checkbox and must accept them to order.
                  Editing them asks anyone with the order page already open to reload.
                  Leave blank for none. {editOrganiserTerms.length}/{ORGANISER_TERMS_MAX}
                </p>
              </div>
