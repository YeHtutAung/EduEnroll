-- ─── 074: Rename MMK-suffixed columns to currency-agnostic names ─────────────
--
-- Column renames:
--   classes.fee_mmk              → classes.fee_amount
--   enrollment_items.fee_mmk     → enrollment_items.fee_amount
--   payments.amount_mmk          → payments.amount
--   payments.received_amount_mmk → payments.received_amount
--
-- Stored functions referencing these columns are recreated below.
...
ALTER TABLE public.classes
  RENAME COLUMN fee_mmk TO fee_amount;

ALTER TABLE public.enrollment_items
  RENAME COLUMN fee_mmk TO fee_amount;

ALTER TABLE public.payments
  RENAME COLUMN amount_mmk TO amount;

ALTER TABLE public.payments
  RENAME COLUMN received_amount_mmk TO received_amount;

-- ── 2. Recreate submit_enrollment() ──────────────────────────────────────────
-- Was: 040_multi_ticket_support.sql — updated column refs and JSON keys.
...
-- ── 3. Recreate submit_cart_enrollment() ──────────────────────────────────────
-- Was: 042_cart_checkout.sql — updated column refs and JSON keys.
...
-- ── 4. Recreate get_tenant_revenue() ─────────────────────────────────────────
-- Was: 066_create_get_tenant_revenue.sql — updated column ref.
